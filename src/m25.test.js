...
describe("M4 #25 chrome", () => {
  ...
  it("puts 이미지·회전(왼쪽/오른쪽) in the same overflow card, 미리보기는 바로 (#106)", () => {
    ...
    assert.doesNotMatch(more, /책갈피|빈 쪽/);
    assert.doesNotMatch(toolbar, /책갈피|이미지|회전/);
    assert.match(drawer, /책갈피/);
    assert.match(drawer, /목차/, "#107: 탭은 목차");
    assert.match(drawer, /빈 쪽/, "#107: 거르기·넣기는 빈 쪽");
  ...
  it("unhides select-layer and float-bar when select indices exist", () => {
    ...
    assert.doesNotMatch(hud, /id="sel-left-btn"|id="sel-right-btn"|>왼쪽<|>오른쪽</);
    assert.ok(hud.indexOf("복사") < hud.indexOf('id="delete-btn"'));
    assert.match(html, /data-handle="rotate"/);
    assert.match(css, /\[data-handle="rotate"\] \{[\s\S]*width: 16px;[\s\S]*height: 16px;[\s\S]*top: -20px;[\s\S]*border: 1\.6px solid #2C2A26;[\s\S]*border-radius: 50%/);
    assert.match(more, /data-rotate="-90">왼쪽/);
    assert.match(more, /data-rotate="90">오른쪽/);
    assert.doesNotMatch(toolbar, /id="sel-left-btn"|id="sel-right-btn"|id="delete-btn"|data-handle="rotate"/);
    assert.match(main, /rotateSelectedItems/);
    assert.match(main, /deleteSelectedItems/);
    assert.match(main, /mode: "rotate"/);
    assert.match(main, /beginRotateFromHandle/);
    assert.doesNotMatch(main, /rotateSelection\(-90\)|rotateSelection\(90\)/);
