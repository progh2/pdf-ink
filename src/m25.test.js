...
describe("M4 #25 chrome", () => {
  ...
  it("unhides select-layer and float-bar when select indices exist", () => {
    ...
    assert.match(hud, /id="sel-left-btn">왼쪽/);
    assert.match(hud, /id="sel-right-btn">오른쪽/);
    assert.match(hud, /id="delete-btn">삭제/);
    assert.ok(hud.indexOf("복사") < hud.indexOf('id="sel-left-btn"'));
    assert.ok(hud.indexOf('id="sel-left-btn"') < hud.indexOf('id="delete-btn"'));
    assert.match(more, /data-rotate="-90">왼쪽/);
    assert.match(more, /data-rotate="90">오른쪽/);
    assert.doesNotMatch(toolbar, /id="sel-left-btn"|id="sel-right-btn"|id="delete-btn"/);
    assert.match(main, /rotateSelectedItems/);
    assert.match(main, /deleteSelectedItems/);
    assert.match(main, /rotateSelection\(-90\)/);
    assert.match(main, /rotateSelection\(90\)/);
