...
describe("M4 #25 chrome", () => {
  ...
  it("unhides select-layer and float-bar when select indices exist", () => {
    ...
    assert.doesNotMatch(hud, /sel-left-btn|sel-right-btn|>왼쪽<|>오른쪽</);
    assert.match(hud, /id="delete-btn">삭제/);
    assert.ok(hud.indexOf("복사") < hud.indexOf('id="delete-btn"'));
    assert.match(html, /id="select-rotate"/);
    assert.match(html, /class="select-rotate-handle"/);
    assert.match(more, /data-rotate="-90">왼쪽/);
    assert.match(more, /data-rotate="90">오른쪽/);
    assert.doesNotMatch(toolbar, /id="sel-left-btn"|id="sel-right-btn"|id="delete-btn"|id="select-rotate"/);
    assert.match(main, /rotateSelectedItems/);
    assert.match(main, /deleteSelectedItems/);
    assert.match(main, /beginRotateDrag/);
    assert.match(main, /mode: "rotate"/);
    assert.doesNotMatch(main, /rotateSelection\(-90\)/);
    assert.doesNotMatch(main, /rotateSelection\(90\)/);
    assert.match(main, /event\.key === "Delete"/);
    assert.match(main, /event\.key === "Backspace"/);
    assert.doesNotMatch(main, /confirmDelete|삭제할까요|window\.confirm/);
    ...
    assert.match(css, /\.select-rotate-handle \{[\s\S]*width: 16px;[\s\S]*height: 16px/);
    assert.match(css, /\.select-rotate-handle \{[\s\S]*top: -20px/);
    assert.match(css, /\.select-rotate-handle \{[\s\S]*border: 1\.6px solid #2[cC]2[aA]26/);
    assert.doesNotMatch(toolbar, /id="select-rotate"|회전/);
    assert.ok(toolbar.indexOf('id="eraser-btn"') < toolbar.indexOf('id="select-btn"'));
    assert.ok(toolbar.indexOf('id="select-btn"') < toolbar.indexOf('id="undo-btn"'));
