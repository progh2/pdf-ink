...
/**
 * #360: 교차 오염 청소. 병합 복사는 id를 보존하고 정당한 복사(#290/#318)는 새
 * id를 받으므로, 다른 문서에 같은 키로 존재하는 항목 = 오염분이다.
 */
export function crossDocKeys(pages, othersPages) {
  const foreign = new Set();
  for (const other of othersPages || []) {
    for (const items of Object.values(other || {})) {
      for (const item of items || []) {
        foreign.add(itemKey(item));
      }
    }
  }
  const hit = new Set();
...
export function removeKeysFromPages(pages, keys) {
  const out = {};
  let removed = 0;
  for (const [page, items] of Object.entries(pages || {})) {
    const kept = (items || []).filter((item) => {
      const drop = keys.has(itemKey(item));
      if (drop) {
        removed += 1;
      }
      return !drop;
